...
function Header() {
    const navigationItems = [
        ...
        {
            title: "Events",
            description: "Discover and connect with local events happening around you.",
            items: [
                {
                    title: "Browse Events",
                    href: "/events",
        ...
        <header className="w-full z-40 fixed top-0 left-0 bg-white backdrop-blur-sm border-b border-gray-100">
            <div className="container relative mx-auto min-h-20 flex gap-4 flex-row lg:grid lg:grid-cols-3 items-center px-4">
                                    ...
                                    {item.href ? (
                                        <>
                                            <NavigationMenuLink asChild>
                                                <Link href={item.href}>
                                                    <Button variant="ghost" className="text-base font-medium text-gray-700 hover:text-[#7C3AED] hover:bg-purple-100">{item.title}</Button>
                                                </Link>
                                            </NavigationMenuLink>
                                        </>
                                    ) : (
                                            ...
                                            <NavigationMenuContent className="!w-[450px] p-4">
                                                <div className="flex flex-col lg:grid grid-cols-2 gap-4">
                                                    <div className="flex flex-col h-full justify-between">
                                                        ...
                                                        <Button asChild size="sm" className="mt-10 bg-[#7C3AED] hover:bg-[#6D28D9] text-white">
                ...
                <div className="flex lg:justify-center">
                    <Link href="/" className="font-semibold text-[#7C3AED] text-2xl">Event Sphere</Link>
                </div>
                <div className="flex justify-end w-full gap-4">
                    <Button variant="ghost" asChild className="hidden md:inline text-base font-medium text-gray-700 hover:text-[#7C3AED] hover:bg-purple-100">
                        <a href="mailto:akshatsing11@gmail.com?subject=Create%20New%20Event&body=Hello%2C%20I'd%20like%20to%20create%20a%20new%20event!">Create Event</a>
                    </Button>
                    <div className="border-r border-purple-200 hidden md:inline"></div>
                    <Button asChild className="text-base font-medium bg-[#7C3AED] hover:bg-[#6D28D9] text-white">
                        <Link href="/events">Get Started</Link>
                    </Button>
                </div>
                <div className="flex w-12 shrink lg:hidden items-end justify-end">
                    <Button variant="ghost" onClick={() => setOpen(!isOpen)} className="hover:bg-purple-100">
                        {isOpen ? <X className="w-6 h-6 text-[#7C3AED]" /> : <Menu className="w-6 h-6 text-[#7C3AED]" />}
                    </Button>
                    {isOpen && (
